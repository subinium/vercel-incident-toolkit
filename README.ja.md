...
> ⚠️ **ご注意**。公式ツールではなく、完全な答えでもなく、思考の代わりにもなりません。[Vercel 2026年4月のセキュリティインシデント](https://vercel.com/kb/bulletin/vercel-april-2026-security-incident)の直後、一人のエンジニアが急ぎでまとめた**ガイドラインスキル**(構造化されたチェックリスト + *任意の* CLI 自動化)です。`--apply` を付ける前に必ず各スクリプトを読んでください。自己責任でご利用ください。最終的な権威は常に Vercel 公式ドキュメント(本文に都度リンク)です。

> 🤖 **AI に丸投げして席を立たないでください。** この toolkit はあなたの本番 Vercel アカウントに対して動作します。`--apply` の前に必ずスクリプトを精査してください — ご自身で、または *AI と一緒に* 各スクリプトの挙動を説明させ、その計画の根拠を問い、upstream `main` との差分を確認しながら。「AI が大丈夫と言った」は GO サインではなく、チェックポイントに過ぎません。すべての破壊的操作はあなたの判断です。
...
- **ローカルの git リポジトリはスキャンしません。** ローカルパスは `scripts/ignore-setup.py` に明示的にパスを渡した時のみ触れます。
- **`api.vercel.com` 以外のホストとは通信しません。**
- **shell rc ファイル・システムキーチェーン・グローバル設定を変更しません。**

## 使い方は 2 通り — ステップごとに選べます

| モード | やり方 | 向くケース |
|---|---|---|
| **自動(CLI)** | スクリプトを `--apply` で実行 | dry-run の出力を信頼し、toolkit にミューテーションを任せたい |
| **手動(リファレンス)** | `scripts/audit.py`(常に読み取り専用)と `scripts/handoff-gen.py` を実行し、残りは Vercel ダッシュボードとベンダーダッシュボードで手動 | *何を*変えるべきかは toolkit に教えてもらい、実際の変更は自分の手で行いたい |

全体を一つのモードに統一する必要はありません — ステップ毎に混ぜて OK。よくある選択: Flow C の内部ランダムローテーションは自動、外部ベンダーローテーションは手動。すべての破壊的スクリプトは **デフォルトで dry-run**、変更前に必ず `y/N` 確認します。
